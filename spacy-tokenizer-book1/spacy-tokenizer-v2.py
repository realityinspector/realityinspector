...
with open("book-whole.txt", "r") as f:
    text = f.read()
...
for tokenized_chunk in tokenized_chunks:
    input_ids = tokenizer.encode(tokenized_chunk, add_special_tokens=True, return_tensors="pt", max_length=1024, truncation=True)
    with torch.no_grad():
...
np.savez("book_embeddings-2.npz", embeddings=embeddings)
